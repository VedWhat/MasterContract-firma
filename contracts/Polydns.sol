...
contract Polydns is ERC721URIStorage {
  ...
  //ved.mcoffee => 0xcasfdaf
  mapping(string => address) public domains;
  //ved.mcoffee => vedChitnis
  ...
  constructor(string memory _tld) payable ERC721("Metacofee Name service", "MNS"){
      tld = _tld;
      console.log("%s name service online", tld);
  ...
  function register(string calldata name) public payable{
      ...
      bytes(
        string(
          abi.encodePacked(
            '{"name": "',
            _name,
            '", "description": "A domain on the Metacoffee name service", "image": "data:image/svg+xml;base64,',
    ...
    reverseDomains[msg.sender] = name;
  ...
  function getDomain() public view returns(string memory){
    return reverseDomains[msg.sender];
  }
}
...
/**
  address => wallet
  domain => ENS domain
  username => ENS domain - .xyz
  IPFSHash => string Ipfs hash 

  registerUser(string domain,string username, address userAddress) => register to userAddress
  registerIpfs(string address,string IPFShash) => register to address
  getIpfsHash(address userAddress) => return Ipfshash
